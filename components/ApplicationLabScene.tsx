...
export default function ApplicationLabScene({ children }: ApplicationLabSceneProps) {
  ...
  const [scenario, setScenario] = useState<{ question: string; focus: string } | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Clear practice scenario state on unmount
      setScenario(null);
      setUserAnswer('');
      setFeedback(null);
...
Cases: ${analysis.cases.map(c => c.name).join(', ')}
Doctrines: ${analysis.doctrines.map(d => d.name).join(', ')}

Full Analysis:
${analysis.topics.map(t => `Topic: ${t.name} - ${t.description}`).join('\n')}
${analysis.cases.map(c => `Case: ${c.name} - ${c.summary}`).join('\n')}
${analysis.doctrines.map(d => `Doctrine: ${d.name} - ${d.explanation}`).join('\n')}
`;
      ...
      // Parse scenario (expecting JSON with question and focus)
      try {
        const scenarioData = JSON.parse(scenarioText);
        setScenario(scenarioData);
      } catch (e) {
        // If not JSON, use as plain text
        setScenario({ question: scenarioText, focus: 'Apply the relevant legal doctrines' });
      }
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          messages: [{
            role: 'user',
            content: `Scenario: ${scenario.question}\n\nStudent's Answer: ${userAnswer}`
          }],
      ...
      setFeedback('❌ Failed to grade answer. Please try again.');
    } finally {
      setIsGrading(false);
    }
  };
                ...
                <div>
                  ...
                  {!scenario ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '20px' }}>
                      ...
                      >
                        {isGenerating ? '⏳ Generating Scenario...' : '🎲 Generate Practice Scenario'}
                      </button>
                    </div>
                  ) : (
                  // Scenario generated - show scenario and answer input
                  <div>
                    {/* Scenario Display */}
                    <div style={{
                      backgroundColor: '#1F2937',
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <textarea
                        ...
                        placeholder="Apply the relevant doctrines and principles to this scenario..."
                        style={{
                          width: '100%',
                          minHeight: '150px',
                          padding: '12px',
                    ...
                    <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
                      <button
                        ...
                        style={{
                          flex: 1,
                          padding: '12px 24px',
                          backgroundColor: (!userAnswer.trim() || isGrading) ? '#6B7280' : '#10B981',
                          color: 'white',
                          border: 'none',
                          borderRadius: '8px',
                          fontSize: '14px',
